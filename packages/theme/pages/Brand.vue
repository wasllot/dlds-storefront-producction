          ...
          <div class="main section">
            ...
            <div class="sidebar desktop-only">
              ...
              <div class="categories-header">
                          <h2>{{ th.getFacetsFromURL().categorySlug?th.getFacetsFromURL().categorySlug.replace('-', ' ').toUpperCase():'Marcas' }}</h2>
              ...
              <LazyHydrate when-idle>
                  <SfLoader :class="{ 'loading--categories': loading }" :loading="loading">
                    ...
                    >
                      ...
                      <SfList class="list brand-list" v-if="brandList">
                        <SfListItem
                            class="list__item"
                            v-for="(brand, j) in brandList.items"
                            :key="j"
                        >
                            <SfMenuItem
                            :count="brand.count || ''"
                            ...
                            <template #label="{ label }">
                                <nuxt-link
                                :to="localePath('/marcas/'+brand.slug)"
                                :class="brand.isCurrent ? 'sidebar--cat-selected' : ''"
                                >
                                {{ brand.label }}
                                </nuxt-link>
                            </template>
            ...
            <SfLoader :class="{ loading }" :loading="loading">
              ...
              <div class="products" v-if="!loading">
                ...
                <div class="category-header" v-if="th.getFacetsFromURL().categorySlug && getCatBanner(th.getFacetsFromURL().categorySlug.toLowerCase())">

                  <img :src="getCatBanner(th.getFacetsFromURL().categorySlug.toLowerCase())" alt="">
...
export default {
  ...
  setup(props, context) {
    ...
    const brandList = computed(() => facetGetters.getBrandTree(productResult.value));
    ...
    onSSR(async () => {
      ...
      await productSearch({ 
        ...
        slug: th.getFacetsFromURL().categorySlug?th.getFacetsFromURL().categorySlug.replace('-', ' '): 'all'
      });
    ...
    const getCatBanner = (category) => {

      if(category == 'grow') return '/banners/categorias/'+category.toLowerCase()+'.jpg';
