<template>
  <SfTabs :open-tab="1">
    <SfTab title="Mis pedidos">
      <div v-if="currentOrder">
        <SfButton class="sf-button--text all-orders" @click="backToAllOrders">Todos los pedidos</SfButton>
      ...
      <SfLoader :class="{ loading }" :loading="loading">
        <div v-if='currentOrder === null'>
          <p class="message">
            Detalle y estado de los pedidos
          </p>
          <div v-if="ordersList.length == 0" class="no-orders">
            <p class="no-orders__title">No tiene pedidos</p>
            <nuxt-link :to="localePath({ name: 'home' })">
              <SfButton class="no-orders__button">Ir a la tienda</SfButton>
            </nuxt-link>
          </div>
          ...
          <p>Pedidos totales - {{ totalOrders }}</p>
        </div>
      </SfLoader>
    </SfTab>
    <SfTab title="Devoluciones">
      <p class="message">
        Proximamente
      </p>
    </SfTab>
